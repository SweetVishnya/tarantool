 ...
 */
static struct uri_query_param *
uri_find_query_param(const struct uri *uri, const char *name)
{
...
/**
 * Add new @a value to URI query @a param.
 */
static void
uri_query_param_add_value(struct uri_query_param *param, const char *value)
{
...
/**
 * Destroy URI query @a param and free all associated resources.
 */
static void
uri_query_param_destroy(struct uri_query_param *param)
{
...
/**
 * Create new URI query @a param, with given @a name.
 */
static void
uri_query_param_create(struct uri_query_param *param, const char *name)
{
...
/**
 * Add new query parameter to @uri structure with given @a name.
 */
static struct uri_query_param *
uri_add_query_param(struct uri *uri, const char *name)
{
	assert(uri_find_query_param(uri, name) == NULL);
	size_t size = (uri->param_count + 1) * sizeof(struct uri_query_param);
...
/**
 * Destroy all @a uri query parameters and free all resources associated
 * with them.
 */
static void
uri_destroy_query_params(struct uri *uri)
{
	for (int i = 0; i < uri->param_count; i++)
		uri_query_param_destroy(&uri->params[i]);
	free(uri->params);
}

/**
 * Create query parameters for @a uri from @a query string. Expected @a
 ...
 */
static void
uri_create_query_params(struct uri *uri, const char *query)
{
	...
	while (optstr != NULL) {
		...
		struct uri_query_param *param =
			uri_find_query_param(uri, name);
		if (param == NULL)
			param = uri_add_query_param(uri, name);
		if (value != NULL)
...
uri_create(struct uri *uri, const char *str)
{
	...
	if (uri->query != NULL)
		uri_create_query_params(uri, uri->query);
	return 0;
}
...
static int
uri_set_add_uri(struct uri_set *uri_set, const char *str)
{
      size_t size = (uri_set->uri_count + 1) * sizeof(struct uri);
      uri_set->uris = xrealloc(uri_set->uris, size);
      struct uri *uri = &uri_set->uris[uri_set->uri_count++];
      return uri_create(uri, str);
}
...
uri_set_create(struct uri_set *uri_set, const char *str)
{
	uri_set->uris = NULL;
	uri_set->uri_count = 0;
	return uri_set_add_uris(uri_set, str);
}
...
uri_set_add_uris(struct uri_set *uri_set, const char *str)
{
	if (str == NULL)
		return 0;
	int uri_count = uri_set->uri_count;
	char *copy = xstrdup(str);
	char *saveptr, *uristr = strtok_r(copy, ",", &saveptr);
	while (uristr != NULL) {
		if (uri_set_add_uri(uri_set, uristr) != 0)
			goto fail;
		uristr = strtok_r(NULL, ", ", &saveptr);
	}
	free(copy);
	return 0;
fail:
	free(copy);
	while (--uri_set->uri_count >= uri_count)
		uri_destroy(&uri_set->uris[uri_set->uri_count]);
	if (++uri_set->uri_count == 0) {
		free(uri_set->uris);
		uri_set->uris = 0;
	}
	return -1;
