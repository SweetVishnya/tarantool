...
base_index_mt.fselect = function(index, key, opts, fselect_opts)
    ...
    local function get_opt(name, default, expected_types)
        ...
        for i,v in pairs(variants) do
            ...
            if string.find(expected_types, ',' .. type(v) .. '.') and
               (i < min_i or min_v == nil) then
                min_i = i
                min_v = v
            end
        end
    ...
    if type(columns) == 'table' then
        ...
        for _,v in ipairs(columns) do
            local split = type(v) == 'string' and string.split(v, ',') or {v}
            for _,str in ipairs(split) do
                if tonumber(str) then
                    table.insert(res_columns, tonumber(str))
                else
                    table.insert(res_columns, string.strip(str))
                end
            end
        end
        columns = res_columns
