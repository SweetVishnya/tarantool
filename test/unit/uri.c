...
#define URI_MAX 10
#define URI_QUERY_PARAM_MAX 10
#define URI_QUERY_PARAM_VALUE_MAX 10

struct uri_query_param_expected {
	/** URI parameter name */
	const char *name;
	/** Count of URI parameter values */
	int value_count;
	/** Expected URI parameter values */
	const char *values[URI_QUERY_PARAM_VALUE_MAX];
};
...
struct uri_expected {
	...
	struct uri_query_param_expected params[URI_QUERY_PARAM_MAX];
};
...
static int
uri_query_param_expected_check(const struct uri_query_param_expected *param,
			       const struct uri *uri)
{
	plan(1 + param->value_count);
	int value_count = uri_query_param_count(uri, param->name);
	is(param->value_count, value_count, "value count");
	for (int idx = 0; idx < MIN(value_count, param->value_count); idx++) {
		const char *value =
			uri_query_param(uri, param->name, idx);
...
uri_expected_check(const struct uri_expected *uri_ex, const struct uri *uri)
{
	...
	for (int i = 0; i < MIN(uri_ex->param_count, uri->param_count); i++)
		uri_query_param_expected_check(&uri_ex->params[i], uri);
	return check_plan();
}

static int
uri_set_expected_check(const struct uri_set_expected *uri_set)
{
	struct uri_set u;
	int rc = uri_set_create(&u, uri_set->string);
	plan(1 + uri_set->uri_count);
	is(rc, 0, "%s: parse", uri_set->string);
...
test_string_uri_set_with_query_params_parse(void)
{
	const struct uri_set_expected uri_set_array[] = {
		...
		/**
		 * Same as previous but with extra ','.
		 */
		[2] = {
			.string = "/unix.sock?q1=v1,, /unix.sock?q2=v2",
			.uri_count = 2,
	...
	for (unsigned i = 0; i < lengthof(uri_set_array); i++)
		uri_set_expected_check(&uri_set_array[i]);
	return check_plan();
}
