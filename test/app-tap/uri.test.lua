...
local function test_parse_uri_set_with_query_params(test)
    -- Tests for uri.parse_set() Lua bindings (several URIs with query parameters).
    -- Parser itself is tested by test/unit/uri unit test.
    test:plan(59)
    ...
    uri_set = uri.parse_set("/tmp/unix.sock?q=v")
    ...
    uri_set = uri.parse_set("/tmp/unix.sock?q=1, /tmp/unix.sock?q=2")
    ...
    uri_set = uri.parse_set("/tmp/unix.sock?q1=1&q1=&q2&q3=" .. "," ..
    ...
    -- Check taht we can't parse string with multiple URIs,
    -- using old method.
